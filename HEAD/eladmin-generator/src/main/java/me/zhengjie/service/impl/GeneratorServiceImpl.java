...
public class GeneratorServiceImpl implements GeneratorService {
    ...
    public void generator(GenConfig genConfig, List<ColumnInfo> columns) {
        if (genConfig.getId() == null) {
            throw new BadRequestException(SysErrLev.FUNCTION_PARAMETER_ERROR, "請先配置產生器");
        }
        try {
            GenUtil.generatorCode(columns, genConfig);
        } catch (IOException e) {
            log.error(e.getMessage(), e);
            throw new BadRequestException(SysErrLev.FUNCTION_PARAMETER_ERROR, "產生失敗，請手動處理已產生的檔案");
        }
    }
    ...
    @Override
    public void download(GenConfig genConfig, List<ColumnInfo> columns, HttpServletResponse response) {
        if (genConfig.getId() == null) {
            throw new BadRequestException(SysErrLev.FUNCTION_PARAMETER_ERROR, "請先配置產生器");
        }
        try {
            ...
            FileUtil.downloadFile(response, new File(zipPath), true);
        } catch (IOException e) {
            throw new BadRequestException(SysErrLev.FUNCTION_PARAMETER_ERROR, "打包失敗");
        }
    }
}
