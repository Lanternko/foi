...
//                .version("2.6")
    ...
    private List<SecurityContext> securityContexts() {
        ...
        // ^(?!auth).*$ 表示所有包含auth的介面不需要使用securitySchemes即不需要帶token
        // ^標識開始  ()里是一子表達式  ?!/auth表示匹配不是/auth的位置，匹配上則新增請求頭，注意路徑已/開頭  .表示任意字元  *表示前面的字元匹配多次 $標識結束
...
class SwaggerDataConfig {
    ...
    private static class Page {
        ...
        @ApiModelProperty("以下列格式排序標準：property[,asc | desc]。 預設排序順序為升序。 支援多種排序條件：如：id,asc")
