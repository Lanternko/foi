...
import foi_wsdoc.WsDocProcessRequest;
import foi_wsdoc.WsDocProcessResponse;
...
public class WsDocService {
	...
	private final String NAMESPACE = "foi-wsdoc";
...
//		2.查傳入帳號下未結案(is_close=false)的案件
	...
	/**
	 * 輸入參數:<br>
	 * String[] params = {"token", "caseno", "Process", "DocNO", "SD", "ED", "dd", "Rd", "other", "ReviceD"};<br>
	 * <br>
	 * 回傳參數:<br>
	 * String msg<br>
	 * 成功時為空字串，異常時填入異常訊息
	 */
	@PayloadRoot(namespace = NAMESPACE, localPart = "wsDocProcessRequest")
	@ResponsePayload
	public WsDocProcessResponse process(@RequestPayload WsDocProcessRequest request) {
//		1.驗證token
//		2.取得案管動作代碼
//		  2.1.確認是否存在公文參數
//		  2.2.取得動作代碼
//		3.載入評議代碼對應實作
		...
		WsDocProcessResponse res = new WsDocProcessResponse();
		String[] params = request.getParams().toArray(new String[0]);
		
		checkToken(params[0]);
		
//		2.1
		String docActionCode = StringUtils.isEmptyOrNull(params[8]) ? params[2] : 
							String.format("%s.%s", params[2], params[8]);										// 有公文參數時格式=評議代號.公文參數

//		2.2
		CodeDetailDto code = codeDetailMap.get(FoiConstants.WS, docActionCode);
		if (code == null || StringUtils.isNotUsed(code.getProperty1())) {
			res.setMsg(String.format("評議代碼: %s 未設定或無須執行動作", docActionCode));
			return res;
		}
...
//		3
		...
		try {
			action = factory.getBean(code.getProperty1(), ProcessAction.class);									// 依名稱取得實作
		} catch (Exception e) {
			res.setMsg(String.format("評議代碼: %s 找不到對應實作: %s", docActionCode, code.getProperty1()));
			return res;
		}

		res.setMsg(action.run(params, code));
		return res;
	}
...
//	驗證來自公文系統的token
	private void checkToken(String key) {
		if ("iAmToken".equals(key)) {		// 開發階段略過token檢查
			return;
		}
