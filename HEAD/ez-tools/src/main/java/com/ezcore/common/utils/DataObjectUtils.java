...
public class DataObjectUtils {
	...
	public static <T> T getItem(T dataObj , String colName ) {
		...
		String methodName   = "";
		try {
			methodName   	= getMethodName(fromClazz, methodStr  );
			Method m1 		= fromClazz.getDeclaredMethod(methodName);
			Object retValue	= m1.invoke(dataObj);

			return (T) retValue;
		}catch(IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException | SecurityException  e ) {
			throw new RuntimeException(String.format("無法完成物件取值!! %s.%s" , fromClazz.getName()) );
		}
	}
	...
	public static <T> T setItem(T dataObj  , String colName  , Object newValue) {
		...
		String colDataType  = "";
		//GetType
		colDataType  = getFieldType(fromClazz , colName) ;
		methodName   = getMethodName(fromClazz, methodStr  );
		try {
